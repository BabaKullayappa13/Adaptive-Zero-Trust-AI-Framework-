...
export default function Navbar({ user, onLogout }: NavbarProps) {
  ...
  return <>
    ...
    <header className="sticky top-0 z-20 flex items-center justify-between border-b border-white/[.08] bg-[#050816]/95 px-4 py-3 backdrop-blur-xl lg:hidden"><Link href="/dashboard" className="flex items-center gap-2 text-sm font-bold text-slate-100"><ShieldCheck className="size-5 text-cyan-300" />AZT <span className="text-cyan-300">AI</span></Link><div className="flex items-center gap-1"><button type="button" aria-label="Notifications" className="rounded-lg p-2 text-slate-400 hover:bg-white/[.06] hover:text-cyan-300"><Bell className="size-4" /></button><button type="button" aria-label="Log out" onClick={logout} className="rounded-lg p-2 text-slate-400 hover:bg-rose-400/10 hover:text-rose-300"><LogOut className="size-4" /></button></div></header>
    <nav className="fixed inset-x-4 bottom-4 z-30 flex items-center justify-around rounded-2xl border border-white/10 bg-[#101b30]/95 p-2 shadow-2xl backdrop-blur-xl lg:hidden" aria-label="Mobile navigation">{links.slice(0, 5).map(({ href, label, icon: Icon }) => <Link key={href} href={href} aria-label={label} className={`flex size-11 items-center justify-center rounded-xl ${active(href) ? 'bg-cyan-300/15 text-cyan-200' : 'text-slate-500'}`}><Icon className="size-5" /></Link>)}</nav>
  </>
}
