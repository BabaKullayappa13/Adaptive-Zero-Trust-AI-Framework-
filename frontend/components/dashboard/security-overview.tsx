...
export default function SecurityOverview({ data, trustScore }: { data: SecurityOverviewData; trustScore?: { score: number; factors?: Record<string, number> } | null }) {
  ...
  return <>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[['Active sessions', data.active_sessions, 'text-cyan-300'], ['Blocked sessions', data.blocked_sessions, 'text-rose-300'], ['Policy violations', data.policy_violations, 'text-amber-300'], ['Federated round', data.federated_learning.round, 'text-emerald-300']].map(([label, value, tone]) => <article className="soc-panel soc-kpi p-5" key={String(label)}><p className="eyebrow">{label}</p><p className={`data-number mt-4 font-mono text-3xl font-bold ${tone}`}>{value}</p><p className="mt-2 text-xs text-slate-500">Backend reported metric</p></article>)}</section>
